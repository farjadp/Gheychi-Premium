from config import BOT_USERNAME

MESSAGES = {
    "fa": {
        "bot_start": "سلام! به بات دانلودر خوش اومدی.\n\nکافیه لینک ویدئو رو بفرستی تا برات دانلود کنم.\n\nپلن فعلی شما: *{plan_name}*\n\n*پلتفرم‌های پشتیبانی‌شده:*\n{platforms}\n\nحداکثر حجم فایل: *{max_mb} مگابایت*\n\nدستورها:\n/menu - نمایش منوی سریع\n/plans - مشاهده پکیج‌ها\n/myplan - مشاهده پلن فعلی\n/mylogs - مشاهده لاگ‌های شخصی\n/usage - مشاهده سهمیه مصرف\n/myid - مشاهده آیدی عددی تلگرام\n/lang - تغییر زبان (Change Language)\n/support - تماس با پشتیبانی",
        "menu_ready": "منوی سریع آماده است.\nاز دکمه‌های زیر یا commandهای بات استفاده کن.",
        "my_id": "Telegram User ID شما:\n`{user_id}`",
        "my_plan": "پلن فعلی: *{effective_plan}*\nپلن ثبت‌شده: {assigned_plan}\nقیمت: ${price}/ماه\nانقضا: `{expiry}`",
        "usage_header": "*{plan_name}*\nانقضا: `{expiry}`",
        "unlimited": "نامحدود",
        "no_logs": "هنوز لاگ شخصی برای شما ثبت نشده است.",
        "last_events": "آخرین رویدادهای شما",
        "support_not_set": "اطلاعات پشتیبانی هنوز در سرور تنظیم نشده است.",
        "support_telegram": "برای پشتیبانی با {contact} در تماس باش.",
        "support_link": "برای پشتیبانی از لینک زیر استفاده کن.",
        "support_text": "پشتیبانی: {contact}",
        "downloads_disabled": "دانلود موقتاً از پنل مدیریت غیرفعال شده است.",
        "invalid_url": "لینک معتبری پیدا نکردم. لطفاً یک URL بفرست.",
        "fetching_info": "در حال دریافت اطلاعات ویدئو...",
        "fetch_error": "اطلاعات این لینک در حال حاضر قابل دریافت نیست. لطفاً کمی بعد دوباره تلاش کن یا لینک دیگری بفرست.",
        "platform_disabled": "این پلتفرم در پنل مدیریت غیرفعال شده است.",
        "expired_request": "این درخواست منقضی شده. لینک را دوباره بفرست.",
        "wrong_user": "این دکمه برای درخواست شما نیست.",
        "downloading": "در حال دانلود... لطفاً صبر کن.",
        "download_progress": "در حال دانلود... {pct}%",
        "download_error": "دانلود این لینک در حال حاضر ممکن نیست. لطفاً کمی بعد دوباره تلاش کن یا لینک دیگری بفرست.\n\nاگر مشکل ادامه داشت، با پشتیبانی تماس بگیر.",
        "download_error_youtube": "دانلود از یوتیوب در حال حاضر نیاز به احراز هویت تازه دارد. تیم پشتیبانی در حال بررسی است؛ لطفاً کمی بعد دوباره تلاش کن یا لینک دیگری بفرست.",
        "sending_file": "دانلود شد، در حال ارسال فایل...",
        "send_error": "فایل دانلود شد، اما ارسال آن در تلگرام با مشکل روبه‌رو شد. لطفاً کمی بعد دوباره تلاش کن.",
        "stripe_not_set": "❌ سیستم پرداخت استرایپ هنوز در پنل ادمین تنظیم نشده است.",
        "invalid_package": "❌ این پکیج نامعتبر است یا امکان خرید مستقیم آن وجود ندارد.",
        "invoice_created": "فاکتور رسمی برای **{plan_name}** صادر شد.\n\nمبلغ: `${price}`\n\nلطفاً از طریق دکمه زیر پرداخت را انجام دهید. پس از پرداخت، اشتراک شما بلافاصله فعال می‌شود.",
        "stripe_error": "❌ خطا در ایجاد فاکتور استرایپ:\n`{error}`",
        "lang_prompt": "لطفاً زبان خود را انتخاب کنید:\nPlease choose your language:",
        "lang_changed": "زبان با موفقیت به فارسی تغییر یافت. 🇮🇷",
        
        # Buttons
        "btn_my_id": "آیدی من",
        "btn_my_plan": "پلن من",
        "btn_usage": "مصرف من",
        "btn_logs": "لاگ من",
        "btn_plans": "پکیج‌ها",
        "btn_support": "پشتیبانی",
        "btn_buy": "💳 خرید {name} (${price})",
        "btn_no_plans": "❌ پکیج‌های پولی در حال حاضر غیرفعال هستند",
        "btn_pay_stripe": "💳 انتقال به درگاه پرداخت استرایپ",
        "btn_dl_mp3": "دانلود MP3",
        "btn_best_quality": "بهترین کیفیت",
        "btn_worst_quality": "کمترین حجم",
        "btn_audio_only": "فقط صدا (MP3)",
        "btn_dashboard": "🖥 داشبورد من",
        "btn_lang": "🌐 تغییر زبان",
        
        # Video Info
        "video_caption": "*{title}*\n\nپلتفرم: {platform}\nآپلودر: {uploader}\nمدت: {duration}\n\nکیفیت مورد نظر را انتخاب کن:",
        "unknown": "نامشخص",
        "unknown_duration": "نامشخص",
        "login_link": "لینک ورود به داشبورد شما:\n{link}\n\n⏳ این لینک ۱۰ دقیقه اعتبار دارد و فقط یک بار قابل استفاده است.",

        # اتصال اکانت تلگرام به حساب وب
        "link_confirm": "🔗 می‌خواهی این اکانت تلگرام به حساب *{email}* وصل شود؟",
        "btn_link_confirm": "✅ بله، وصل کن",
        "btn_link_cancel": "لغو",
        "link_success": "✅ این اکانت تلگرام به حساب تو وصل شد.",
        "link_cancelled": "لغو شد. هیچ اتصالی انجام نشد.",
        "link_invalid": "این کد یا لینک نامعتبر است یا منقضی شده. از پنل کاربری یکی تازه بگیر.",
        "link_already_here": "این اکانت تلگرام از قبل به همین حساب وصل است.",
        "link_owned_elsewhere": "این اکانت تلگرام به حساب *{email}* وصل است. اول از آنجا جدایش کن.",
        "link_cap_reached": "پلن فعلی‌ات {cap} اکانت تلگرام را پشتیبانی می‌کند. برای بیشتر، پلن را ارتقا بده.",
        "link_cooldown": "یک اسلات را به‌تازگی آزاد کرده‌ای. تا تاریخ {date} نمی‌توان اکانت تازه‌ای جایش گذاشت.",
        "link_usage": "کد شش‌رقمی را همراه دستور بفرست. مثال:\n`/link 483920`",
        "stripe_product_description": "خرید اشتراک ماهانه {plan_name} ربات تلگرام",
        "file_size_exceeded": "حجم فایل ({size_mb} مگابایت) بیشتر از سقف مجاز دانلود سرور است.",
        "direct_download_fallback": "به دلیل این محدودیت، فایل به صورت مستقیم در تلگرام ارسال نشد. اما می‌توانید ویدیو را مستقیماً از طریق دکمه زیر دانلود کنید:",
        "btn_direct_download": "📥 دانلود مستقیم (مرورگر)",
        "no_direct_link": "متاسفانه لینک دانلود مستقیمی برای این کیفیت در دسترس نیست.",
        "telegram_size_limit": "حجم فایل ({size_mb} مگابایت) بیشتر از سقف مجاز تلگرام ({max_size_mb} مگابایت) است.",
        "document_caption": "📁 {title}\n\n🤖 @{bot}",
        "file_fallback": "فایل",
        
        # Commands descriptions
        "cmd_start": "شروع و نمایش منوی اصلی",
        "cmd_menu": "نمایش منوی سریع",
        "cmd_plans": "مشاهده پکیج‌ها",
        "cmd_myplan": "مشاهده پلن فعلی",
        "cmd_usage": "مشاهده سهمیه و اعتبار",
        "cmd_mylogs": "مشاهده لاگ‌های شخصی",
        "cmd_myid": "دریافت Telegram User ID",
        "cmd_lang": "تغییر زبان بات",
        "cmd_support": "دریافت راهنمای پشتیبانی",
        "cmd_dashboard": "ورود به پنل کاربری وب",

        "admin_panel": "پنل مدیریت",
        "dashboard": "داشبورد",
        "finance": "امور مالی",
        "settings": "تنظیمات",
        "subscriptions": "اشتراک‌ها",
        "plans": "پلن‌ها",
        "broadcast": "پیام سراسری",
        "logs": "لاگ‌ها",
        "saved_success": "تغییرات با موفقیت ذخیره شد",
        "total_events": "کل رویدادها",
        "in_db": "در پایگاه داده",
        "errors": "خطاها",
        "out_of_total": "از کل رویدادها",
        "users": "کاربران",
        "registered": "ثبت‌شده",
        "active_subs": "اشتراک فعال",
        "paid_plan": "پلن پولی",

        # Payment success notification
        "payment_success_title": "🎉 پرداخت با موفقیت انجام شد!",
        "payment_success_body": "اشتراک *{plan_name}* شما فعال شد.\n\n📅 تاریخ انقضا: `{expiry}`\n💳 مبلغ پرداختی: `${price}`\n\n*قابلیت‌های پلن شما:*\n{rules}\n\n*دستورهای کاربردی:*\n• /menu — منوی سریع\n• /usage — مشاهده سهمیه باقی‌مانده\n• /myplan — جزئیات پلن شما\n• /dashboard — ورود به داشبورد وب\n\nکافیست لینک ویدئو را برای بات بفرستید تا دانلود شود. 🚀",
        "payment_success_unlimited_expiry": "بدون محدودیت زمانی",

        # ===== Save Restricted Content (UserBot) =====
        "tg_fetching": "🔐 در حال دریافت محتوای پروتکت‌شده تلگرام...",
        "tg_downloading": "⬇️ در حال دانلود... لطفاً صبر کن.",
        "tg_sending": "📤 در حال ارسال فایل...",
        "tg_no_media": "این پیام فایل رسانه‌ای ندارد که بتوان ارسال کرد.",
        "tg_invalid_link": "لینک تلگرام نامعتبر است. لطفاً یک لینک معتبر از t.me بفرست.",
        "tg_userbot_disabled": "⚠️ فیچر ذخیره محتوای پروتکت‌شده هنوز پیکربندی نشده.\n\nادمین باید TG_API_ID، TG_API_HASH و TG_SESSION_STRING را تنظیم کند.",
        "tg_access_denied": "❌ دسترسی به این کانال یا گروه امکان‌پذیر نیست. اکانت UserBot باید عضو آن باشد.",
        "tg_flood_wait": "⏳ به دلیل محدودیت تلگرام باید کمی صبر کنید. لطفاً {seconds} ثانیه دیگر دوباره تلاش کنید.",
        "tg_download_failed": "❌ دانلود محتوا از تلگرام ناموفق بود. لطفاً دوباره تلاش کن.",
        "tg_album_caption": "📁 {count} فایل از تلگرام",
        "tg_caption": "📁 {title}\n\n🤖 @{bot}",

        # ===== Concurrency =====
        "large_file_relay": "📦 فایل بزرگ‌تر از حد معمول است. در حال ارسال از مسیر جایگزین — کمی بیشتر طول می‌کشد.",
        "user_busy": "⏳ یک دانلود دیگر از شما همین حالا در حال انجام است.\nلطفاً تا پایان آن صبر کن و بعد لینک بعدی را بفرست.",
        "queue_wait": "⏳ سرور در حال حاضر شلوغ است. درخواست شما در صف قرار گرفت..."
    },
    "en": {
        "bot_start": "Hello! Welcome to the Downloader Bot.\n\nJust send me a video link and I'll download it for you.\n\nCurrent Plan: *{plan_name}*\n\n*Supported Platforms:*\n{platforms}\n\nMax File Size: *{max_mb} MB*\n\nCommands:\n/menu - Show quick menu\n/plans - View packages\n/myplan - View current plan\n/mylogs - View personal logs\n/usage - View usage quota\n/myid - Get Telegram User ID\n/lang - Change Language\n/support - Contact Support",
        "menu_ready": "Quick menu is ready.\nUse the buttons below or bot commands.",
        "my_id": "Your Telegram User ID:\n`{user_id}`",
        "my_plan": "Current Plan: *{effective_plan}*\nAssigned Plan: {assigned_plan}\nPrice: ${price}/month\nExpires: `{expiry}`",
        "usage_header": "*{plan_name}*\nExpires: `{expiry}`",
        "unlimited": "Unlimited",
        "no_logs": "No personal logs recorded for you yet.",
        "last_events": "Your recent events",
        "support_not_set": "Support information is not set on the server yet.",
        "support_telegram": "Contact {contact} for support.",
        "support_link": "Use the link below for support.",
        "support_text": "Support: {contact}",
        "downloads_disabled": "Downloads are temporarily disabled from the admin panel.",
        "invalid_url": "No valid link found. Please send a URL.",
        "fetching_info": "Fetching video info...",
        "fetch_error": "This link's info cannot be fetched right now. Please try again later or send another link.",
        "platform_disabled": "This platform is disabled in the admin panel.",
        "expired_request": "This request has expired. Send the link again.",
        "wrong_user": "This button is not for your request.",
        "downloading": "Downloading... Please wait.",
        "download_progress": "Downloading... {pct}%",
        "download_error": "This link cannot be downloaded right now. Please try again later or send another link.\n\nIf the issue continues, contact support.",
        "download_error_youtube": "YouTube downloads currently require refreshed authentication. Support is checking it; please try again later or send another link.",
        "sending_file": "Downloaded, sending file...",
        "send_error": "The file was downloaded, but Telegram could not send it. Please try again later.",
        "stripe_not_set": "❌ Stripe payment system is not configured yet.",
        "invalid_package": "❌ This package is invalid or direct purchase is not available.",
        "invoice_created": "Official invoice created for **{plan_name}**.\n\nAmount: `${price}`\n\nPlease proceed to payment using the button below. Your subscription will be activated immediately after payment.",
        "stripe_error": "❌ Error creating Stripe invoice:\n`{error}`",
        "lang_prompt": "لطفاً زبان خود را انتخاب کنید:\nPlease choose your language:",
        "lang_changed": "Language successfully changed to English. 🇬🇧",
        
        # Buttons
        "btn_my_id": "My ID",
        "btn_my_plan": "My Plan",
        "btn_usage": "My Usage",
        "btn_logs": "My Logs",
        "btn_plans": "Packages",
        "btn_support": "Support",
        "btn_buy": "💳 Buy {name} (${price})",
        "btn_no_plans": "❌ Paid packages are currently disabled",
        "btn_pay_stripe": "💳 Proceed to Stripe Checkout",
        "btn_dl_mp3": "Download MP3",
        "btn_best_quality": "Best Quality",
        "btn_worst_quality": "Smallest Size",
        "btn_audio_only": "Audio Only (MP3)",
        "btn_dashboard": "🖥 My Dashboard",
        "btn_lang": "🌐 Language",
        
        # Video Info
        "video_caption": "*{title}*\n\nPlatform: {platform}\nUploader: {uploader}\nDuration: {duration}\n\nChoose your desired quality:",
        "unknown": "Unknown",
        "unknown_duration": "Unknown",
        "login_link": "Your dashboard login link:\n{link}\n\n⏳ Valid for 10 minutes, and it works only once.",

        # Linking a Telegram account to a web account
        "link_confirm": "🔗 Link this Telegram account to *{email}*?",
        "btn_link_confirm": "✅ Yes, link it",
        "btn_link_cancel": "Cancel",
        "link_success": "✅ This Telegram account is now linked to your account.",
        "link_cancelled": "Cancelled. Nothing was linked.",
        "link_invalid": "That code or link is invalid or has expired. Get a fresh one from your account page.",
        "link_already_here": "This Telegram account is already linked to that account.",
        "link_owned_elsewhere": "This Telegram account belongs to *{email}*. Unlink it there first.",
        "link_cap_reached": "Your plan covers {cap} Telegram account(s). Upgrade to link more.",
        "link_cooldown": "You freed a slot recently. A different Telegram account can take it from {date}.",
        "link_usage": "Send the six-digit code with the command. For example:\n`/link 483920`",
        "stripe_product_description": "Monthly subscription for {plan_name} - Telegram Bot",
        "file_size_exceeded": "File size ({size_mb} MB) exceeds the server download limit.",
        "direct_download_fallback": "Due to this limit, the file was not sent directly in Telegram. However, you can download the video directly using the button below:",
        "btn_direct_download": "📥 Direct Download (Browser)",
        "no_direct_link": "Unfortunately, no direct download link is available for this quality.",
        "telegram_size_limit": "File size ({size_mb} MB) exceeds Telegram's allowed limit ({max_size_mb} MB).",
        "document_caption": "📁 {title}\n\n🤖 @{bot}",
        "file_fallback": "File",
        
        # Commands descriptions
        "cmd_start": "Start and show main menu",
        "cmd_menu": "Show quick menu",
        "cmd_plans": "View packages",
        "cmd_myplan": "View current plan",
        "cmd_usage": "View usage and quota",
        "cmd_mylogs": "View personal logs",
        "cmd_myid": "Get Telegram User ID",
        "cmd_lang": "Change bot language",
        "cmd_support": "Get support info",
        "cmd_dashboard": "Access Web Dashboard",

        "admin_panel": "Admin Panel",
        "dashboard": "Dashboard",
        "finance": "Finance",
        "settings": "Settings",
        "subscriptions": "Subscriptions",
        "plans": "Plans",
        "broadcast": "Broadcast",
        "logs": "Logs",
        "saved_success": "Changes saved successfully",
        "total_events": "Total Events",
        "in_db": "In Database",
        "errors": "Errors",
        "out_of_total": "Out of total",
        "users": "Users",
        "registered": "Registered",
        "active_subs": "Active Subs",
        "paid_plan": "Paid Plan",

        # Payment success notification
        "payment_success_title": "🎉 Payment received successfully!",
        "payment_success_body": "Your *{plan_name}* subscription is now active.\n\n📅 Expires on: `{expiry}`\n💳 Amount paid: `${price}`\n\n*Your plan benefits:*\n{rules}\n\n*Useful commands:*\n• /menu — Quick menu\n• /usage — View remaining quota\n• /myplan — View plan details\n• /dashboard — Open web dashboard\n\nJust send any video link to the bot to start downloading. 🚀",
        "payment_success_unlimited_expiry": "No expiry",

        # ===== Save Restricted Content (UserBot) =====
        "tg_fetching": "🔐 Fetching restricted Telegram content...",
        "tg_downloading": "⬇️ Downloading... Please wait.",
        "tg_sending": "📤 Sending file...",
        "tg_no_media": "This message has no media file to send.",
        "tg_invalid_link": "Invalid Telegram link. Please send a valid t.me link.",
        "tg_userbot_disabled": "⚠️ Save Restricted Content feature is not configured yet.\n\nAdmin needs to set TG_API_ID, TG_API_HASH, and TG_SESSION_STRING.",
        "tg_access_denied": "❌ Cannot access this channel or group. The UserBot account must be a member.",
        "tg_flood_wait": "⏳ Telegram rate limit hit. Please try again in {seconds} seconds.",
        "tg_download_failed": "❌ Failed to download content from Telegram. Please try again.",
        "tg_album_caption": "📁 {count} files from Telegram",
        "tg_caption": "📁 {title}\n\n🤖 @{bot}",

        # ===== Concurrency =====
        "large_file_relay": "📦 This file is larger than usual. Sending it another way — it will take a little longer.",
        "user_busy": "⏳ You already have a download in progress.\nPlease wait for it to finish before sending another link.",
        "queue_wait": "⏳ The server is busy right now. Your request has been queued..."
    }
}

def get_text(key: str, lang: str = "fa", **kwargs) -> str:
    # Fallback to fa if lang not supported
    if lang not in MESSAGES:
        lang = "fa"
    text = MESSAGES[lang].get(key, MESSAGES["fa"].get(key, key))

    # The bot handle is supplied here rather than at every call site: format()
    # failures are swallowed below, so a caller that forgot it would silently
    # ship "{bot}" to the user.
    if "{bot}" in text:
        kwargs.setdefault("bot", BOT_USERNAME)

    if kwargs:
        try:
            return text.format(**kwargs)
        except Exception:
            return text
    return text
